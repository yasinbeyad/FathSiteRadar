...
//			weaponController.tpsSettingsSlot = saveSlot;
		...
		public static void InstantiateWeapon(GameObject weapon, int index, InventoryManager manager, Controller controller)
		{
			...
			// manager.slots[index].weaponSlotInInspector.Add(new WeaponSlotInInventory{weapon = weapon});
...
//			}
		...
		static void InstantiateAdditionalBloodEffects(List<GameObject> effects, Vector3 position, Quaternion rotation, Transform parent = null)
		{
                ...
                if (Mathf.Abs(script.Controller.anim.GetFloat("CameraAngle")) < 60)
                {
	                if (script.Controller.TypeOfCamera == CharacterHelper.CameraType.TopDown || script.Controller.TypeOfCamera == CharacterHelper.CameraType.ThirdPerson && script.Controller.emulateTDModeLikeTP)
	                {
		                if (!script.Controller.CameraParameters.lockCamera)
		                {
			                ...
			                script.attackDirection = script.tempCamera.TransformDirection(Vector3.up + new Vector3(Random.Range(-script.currentBulletsScatter.x, script.currentBulletsScatter.x), Random.Range(-script.currentBulletsScatter.y, script.currentBulletsScatter.y), 0));
		                }
		                else
		                {
			                if (script.Controller.CameraParameters.lookAtCursor)
			                {
				                if (!script.Controller.bodyLimit)
				                {
					                ...
					                script.attackDirection = dir + new Vector3(Random.Range(-script.currentBulletsScatter.x, script.currentBulletsScatter.x), Random.Range(-script.currentBulletsScatter.y, script.currentBulletsScatter.y), Random.Range(-script.currentBulletsScatter.y, script.currentBulletsScatter.y)) * 10;
